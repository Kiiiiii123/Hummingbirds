...
public class HummingbirdAgent : Agent
{
    ...
    public override void OnActionReceived(float[] vectorAction)
    {
        ...
        // Clamp pitch to avoid flipping upside and down.
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Observe a dot product vector indicates whether the beak is pointing toward the flower (1 observation).
    ...
    {
        // Create all placeholders for all movement/turning.
        ...
        // Add the three movement values, pitch and yaw to the actionOut array.
    ...
    /// </summary>
    /// <param name="inFrontOfFlower">Whether to choose a spot in front a flower</param>
    private void MoveToSafeRandomPosition(bool inFrontOfFlower)
    {
        bool safePositonFound = false;
        int attemptsRemaining = 100; // Prevent an infinite loop.
        Vector3 potentialPosition = Vector3.zero;
        Quaternion potentialRotation = new Quaternion();
        
        // Loop until a safe position is found or we run out of attempts.
        while (!safePositonFound && attemptsRemaining > 0)
        {
            attemptsRemaining--;
            if (inFrontOfFlower)
            {
            ...
            safePositonFound = colliders.Length == 0;
        }

        Debug.Assert(safePositonFound, "Could not find a safe position to spawn.");
    ...
    private void TriggerEnterOrStay(Collider collider)
    {
        ...
        if (collider.CompareTag("nectar"))
        {
            ...
            if (Vector3.Distance(beakTip.position, closestPointToBeakTip) < BeakTipRadius)
            {
                ...
                // Attempt to take .o1 nectar.
                // Note: this is per fixed time step, meaning it happens every .02 second, or 50x per second.
                ...
                if (trainingMode)
                {
                    ...
                    // Encourage to point its beak directly at the flower or into the flower.
                    float bonus = .02f * Mathf.Clamp01(Vector3.Dot(transform.forward.normalized,
                        -nearestFlower.FlowerUpVector.normalized));
                    
                    // It's always going to get .01f if the beak is inside the nectar.
                    AddReward(.01f + bonus);
                }
    ...
    {
        // Avoid scenario where nearest flower's nectar is stolen by opponent and not updated.
